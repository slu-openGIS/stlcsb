...
```r
> csbValid <- csb_cancelled(csb, var = datecancelled)
>
> nrow(csb)
[1] 1160418
> nrow(csbValid)
...
## Time and Date
The package offers two functions for handling time and date. The `csb_date_filter()`function is used to sbuset observations for a specified time period. It is flexible in syntax, accepting 3 letter abbreviations, full month names, or a numeric argument:
...
> library(ggplot2)
> ggplot() + 
+   geom_sf(data = vacant_sf, colour = "red", fill = NA, size = .5)
```
