...
`csb_missingXY` is used to remove observations with incomplete or invalid spatial data.
```{r missing, eval=FALSE}
csb_no_missing <- csb_missingXY(csb, SRX, SRY)
```

`csb_projectXY` is used to create a simple features object, which can then be manipulated spatially. It is mandatory that `csb_missingXY` is used prior. The coordinate reference system can be re-projected by specifying a numeric epsg code or a proj4 character string 
```{r geo, eval=FALSE}
...
csb_vacant(csb, PROBLEMCODE, vacant, filter = TRUE) %>%
  ...
  csb_missingXY(SRX, SRY, filter = TRUE) %>%
